"""Generative outpainting utilities."""
...
def expand_image(image: Image.Image, target_width: int, target_height: int, prompt: str = "outpainting") -> Image.Image:
    """Expand ``image`` to ``target_width`` x ``target_height`` using generative outpainting."""
    ...
    x_off = max((target_width - original_width) // 2, 0)
    y_off = max((target_height - original_height) // 2, 0)
